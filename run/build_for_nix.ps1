...
import { Console, bold, lightRed, yellow } from "https://deno.land/x/quickr@0.7.4/main/console.js"
import { run, Timeout, Env, Cwd, Stdin, Stdout, Stderr, Out, Overwrite, AppendTo, throwIfFails, returnAsString, zipInto, mergeInto } from "https://deno.land/x/quickr@0.7.4/main/run.js"
...
await FileSystem.write({
    ...
    `.replace(
        ...
        /REPLACEME_COMMIT_HASH_420492093/g, latestCommitHash
    ).replace(
...
await FileSystem.write({
    ...
    `.replace(/\n    /g, "\n")+(await FileSystem.read(readmeFileTemplate)).replace(/REPLACEME_COMMIT_HASH_420492093/g, latestCommitHash)
})
